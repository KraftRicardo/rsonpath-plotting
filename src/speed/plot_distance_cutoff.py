...
def plot_per_name(plots_dir_path: str, build_csv_path: str, query_csv_path: str, counter_csv_path: str, top_image_path, cutoffs):
    ...
    output_filename = f"{plots_dir_path}/{json_filename}.png"
...
def collect_build(result_dir_path, cutoffs, build_dir_path):
...
def collect_query(result_dir_path, cutoffs, query_dir_path):
...
def plot_all(data_dir_path: str, counter_dir_path: str, top_image_dir_path: str, result_dir_path: str, cutoffs):
    ...
    names = [
        ...
        for f in os.listdir(build_dir_path)
        if f.endswith(".csv")
    ]
...
# "counter_dir_path" is the path to the folder that contains the skip-counter information in .csv files with this
# structure:
...
#   bestbuy_large_record_(1GB),3,0.803025,0.000000,0.803025,459785,0,459785,838855513,0,838855513,1044619305
#   ...
# the name of this .csv is "COUNTER_bestbuy_large_record_(1GB).csv" and there needs to be one .csv per different JSON
# "top_image_dir_path" is the path to the folder that holds the distance distribution plots for the analyzed JSON
# "result_dir_path" is the place where the result plots and their side effect .csv files will be stored
